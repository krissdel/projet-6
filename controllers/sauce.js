        ...
        fs.unlink(`images/${filename}`, () => {
          SaucesModel.updateOne({
            ...
            .then(() => res.status(200).json({
              message: 'mise a jour de la sauce !'
            }))
            .catch(error => res.status(400).json({
              error
            }));
...
exports.likeSauces = (req, res) => {
  ...
  if (req.body.like === 1) { //utilisateur like une sauce
    SaucesModel.updateOne({
        ...
        }, // Ajoute l'id de l'utilisateur à la liste des utilisateurs aimant la sauce
        $inc: {
          likes: 1
        }, // Incrémente de 1 le nombre d'utilisateurs aimant la sauce
      })
      .then(() => res.status(200).json({
        message: "like sauce !"
      }))
  ...
  if (req.body.like === -1) { // utilisateur dislike une sauce
    SaucesModel.updateOne({
        ...
        }, // Ajoute l'id de l'utilisateur à la liste des utilisateurs n'aimant pas la sauce
        $inc: {
          dislikes: 1
        }, // Incrémente de 1 le nombre d'utilisateurs n'aimant pas la sauce
      })
      .then(() => res.status(200).json({
        message: "dislike sauce !"
      }))
      .catch(error => res.status(400).json({
        error
      }));
      ...
      .then(sauce => {
        ...
        if (alreadyLiked) { // Si déjas liké
          SaucesModel.updateOne({
              ...
              $pull: {
                usersLiked: req.body.userId
              }, // Supprime l'id de l'utilisateur de la liste des utilisateurs aimant la sauce
              $inc: {
                likes: -1
              }, // Décrémente de 1 le nombre d'utilisateurs aimant la sauce
            })
            .then(() => res.status(200).json({
          ...
          SaucesModel.updateOne({
              ...
              $pull: {
                usersDisliked: req.body.userId
              }, // Supprime l'id de l'utilisateur de la liste des utilisateurs n'aimant pas la sauce
              $inc: {
                dislikes: -1
              }, // Décrémente de 1 le nombre d'utilisateurs n'aimant pas la sauce
            })
            .then(() => res.status(200).json({
